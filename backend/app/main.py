...
async def lifespan(app: FastAPI):
    print("RiskShield AI Manager starting up...")
    # Initialise optional Copilot service (safe even if Ollama is absent)
    app_state["copilot_service"] = CopilotService(app_state["local_llm"])
    try:
        df = dataset_loader.load_dataset("banksim", normalize=True)
        if df is not None and len(df) > 0:
            print(f"Loaded BankSim dataset: {len(df)} rows")
    ...
    yield
    print("RiskShield AI Manager shutting down...")
